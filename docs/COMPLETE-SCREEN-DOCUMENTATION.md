...
Login → server-issued destination → Tenant JWT → الوجهة حسب الدور. مساحة نشطة واحدة
تدخل مباشرة، وطلب معلق واحد يفتح `/identity/application-status` مباشرة. تظهر قائمة
اختيار مختصرة فقط عند وجود أكثر من وجهة، ولا تظهر شاشة `اختيار السياق` القديمة أو
حالة `ابدأ باستخدام LogicFit` الفارغة.
