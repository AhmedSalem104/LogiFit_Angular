...
| `/identity/application-status` | حامل Tracking Token أو هوية أعادت إصداره | يعرض حالة الطلب وطلب الاستكمال، يحدّث الحالات غير النهائية تلقائيًا كل 10 ثوانٍ، ويوفر تحديثًا يدويًا وزر دخول بعد الجاهزية. يرسل الحقول المطلوبة فقط ثم يعيد التقديم؛ مساحات Gym وFreelanceCoach تستقبل حقول payload المشتركة التي طلبتها الإدارة فقط. | انتهاء الجلسة أو الضغط على العودة يوجّه لتسجيل الدخول بالهوية؛ عند `canAccessDashboard=true` يتوقف التحديث وتظهر `الدخول إلى المنصة` التي تبدأ جلسة اختيار Workspace؛ الرفض لا يعدّل القرار بل يبدأ طلبًا جديدًا. |
