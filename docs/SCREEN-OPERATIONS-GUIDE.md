...
| `/identity/application-status` | حامل Tracking Token أو هوية أعادت إصداره | يعرض حالة الطلب وطلب الاستكمال، يحدّث الحالات غير النهائية تلقائيًا كل 10 ثوانٍ، ويوفر تحديثًا يدويًا وزر دخول بعد الجاهزية. يرسل الحقول المطلوبة فقط ثم يعيد التقديم؛ وعند غياب إثبات الدفع يظهر رفع JPG/PNG/PDF حتى 10MB من نفس النموذج، ثم يستخدم `POST /api/workspace-applications/tracking/payment-proof` بالتوكن المؤقت قبل إعادة الإرسال. | انتهاء الجلسة أو الضغط على العودة يوجّه لتسجيل الدخول بالهوية؛ عند `canAccessDashboard=true` يتوقف التحديث وتظهر `الدخول إلى المنصة` التي تبدأ جلسة اختيار Workspace؛ الرفض لا يعدّل القرار بل يبدأ طلبًا جديدًا. لا يظهر storage key أو PaymentRequestId للمالك. |
...
| `/owner/subscriptions` — اشتراكات العملاء | صلاحية إدارة الاشتراكات. | إصدار ومتابعة وتجديد اشتراك العضو. | بحث بالعضو/الحالة/التاريخ، إنشاء اشتراك لعميل موجود أو إنشاء أول عميل مع اشتراكه من نفس النموذج، مراجعة البداية والنهاية والحالة والمدفوعات المرتبطة. | يفتح النموذج حتى مع قائمة عملاء فارغة، ويمنع إنشاء تضارب غير مسموح أو تجاوز الصلاحية؛ حالة الاشتراك تحدد ما تراه البوابة والعضو. |

#### تدفق إنشاء أول مشترك واشتراك

1. يفتح مالك الجيم «اشتراك جديد» من `/owner/subscriptions` حتى لو لم يوجد أي عميل بعد.
2. عند عدم وجود عملاء يختار النظام تلقائياً «إنشاء عميل جديد مع الاشتراك»؛ يرسل النموذج بيانات العميل والباقة وتاريخ البداية والدفع والبطاقة إلى `POST /api/Clients/onboard`.
3. ينشئ الخادم العميل والملف والعضوية في معاملة واحدة، ثم ينشئ `ClientSubscription` ويربطها بالـTenant، ويصدر بطاقة العضوية عند طلبها.
4. عند اختيار عميل موجود يستخدم النموذج `POST /api/Subscriptions` لإنشاء الاشتراك فقط، مع منع الاشتراك المتداخل للعميل نفسه.
5. عند الفشل تظهر رسالة قابلة للفهم وتبقى البيانات متسقة؛ لا يتم ترك عميل جديد دون اشتراك بسبب فشل مرحلة العضوية.
