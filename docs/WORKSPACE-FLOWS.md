...
- تعرض `/auth/login` رحلة بصرية من خطوتين: تحديد مساحة الصالة من النطاق/المعرّف ثم إدخال رقم الهاتف وكلمة المرور.
- لا تتغير طلبات `AuthService` أو حفظ الجلسة أو انتقال الدور بعد النجاح؛ التغيير تنظيمي ومرئي فقط.
- رابط الدخول بالهوية يظل منفصلاً في `/identity/login` للمستخدم ذي المساحات المتعددة أو الطلبات المعلقة، ولا يخلط هويته العامة مع جلسة صالة تقليدية.
