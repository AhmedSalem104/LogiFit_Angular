...
@Component({
  ...
  template: `
    ...
    @else { <form [formGroup]="form" (ngSubmit)="submit()">
      <label>الاسم الكامل<input class="form-input" formControlName="fullName" autocomplete="name" /></label>
      <label>البريد الإلكتروني<input class="form-input" type="email" formControlName="email" autocomplete="email" dir="ltr" /></label>
      <label>رقم الهاتف (اختياري)
        <span class="phone-row" dir="ltr">
          <select class="form-input" formControlName="countryCode" aria-label="كود الدولة">
          ...
          <input class="form-input" formControlName="phoneNumber" autocomplete="tel-national"
      ...
      <button class="btn btn-primary" [disabled]="saving() || form.invalid">{{ saving() ? 'جارٍ الإنشاء...' : 'إنشاء وإرسال رابط التأكيد' }}</button>
    </form> }
    <p class="link"><a routerLink="/identity/login">لديك حساب بالفعل؟ سجّل الدخول</a></p></section>`,
  styles: [`.identity-register h2{margin:0 0 .5rem;color:var(--text-primary);font-size:1.65rem}.eyebrow{margin:0 0 .35rem;color:#2563eb;font-weight:800;font-size:.75rem;letter-spacing:.08em;text-transform:uppercase}.subtitle{color:var(--text-secondary);line-height:1.7}.identity-register form{display:grid;gap:.8rem;margin-top:1rem}label{display:grid;gap:.35rem;color:var(--text-primary);font-size:.9rem;font-weight:600}.form-input{width:100%;box-sizing:border-box;min-height:42px;padding:.65rem .75rem;border:1px solid var(--border-color);border-radius:8px;color:var(--text-primary);background:var(--bg-primary);font:inherit}.phone-row{display:grid;grid-template-columns:108px 1fr;gap:.5rem}.phone-hint{margin-top:-.5rem;color:var(--text-secondary);line-height:1.6}.btn{min-height:46px}.btn:disabled{opacity:.65}.success{margin-top:1rem;padding:1rem;border:1px solid #86efac;border-radius:.65rem;color:#166534;background:#f0fdf4;display:grid;gap:.4rem;line-height:1.7}.success i{font-size:1.3rem}.error{margin:0;color:#b91c1c}.link{margin-top:1rem}.link a{color:#2563eb}@media(max-width:420px){.phone-row{grid-template-columns:96px 1fr}}`],
})
export class IdentityRegisterComponent {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    countryCode: ['+20'],
    phoneNumber: ['', Validators.pattern(/^[+\d\s()-]{8,22}$/)],
  ...
  submit(): void {
    ...
    this.onboarding.registerIdentity(data.fullName, data.email, data.password,
      data.phoneNumber ? this.normalizedPhone(data.countryCode, data.phoneNumber) : undefined).subscribe({
      next: () => { this.saving.set(false); this.completed.set(true); },
      error: err => {
        this.saving.set(false);
        this.error.set(err?.translatedMessage || err?.error?.message || 'تعذر إنشاء الهوية.');
      },
    });
  }

  private normalizedPhone(countryCode: string, phoneNumber: string): string {
