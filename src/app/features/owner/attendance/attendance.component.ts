...
@Component({
  ...
  template: `
    <div class="attendance-page">
      ...
      <div class="state-card error-state" *ngIf="!loading() && errorMessage()" role="alert">
        ...
        <div>
          <strong>تعذر تحميل سجل الحضور</strong>
          <p>{{ errorMessage() }}</p>
          <button type="button" class="btn btn-primary" (click)="loadAttendance()">إعادة المحاولة</button>
        </div>
      </div>
      ...
      <div class="summary-warning" *ngIf="!errorMessage() && summaryError()" role="status">
        <i class="pi pi-info-circle"></i><span>{{ summaryError() }}</span>
      </div>
  ...
  styles: [`
    ...
    .summary-warning { display:flex; align-items:center; gap:.5rem; margin:-.75rem 0 1.25rem; padding:.65rem .8rem; border:1px solid #fde68a; border-radius:10px; color:#92400e; background:#fffbeb; }
  ...
  loadAttendance(): void {
    ...
    this.attendanceService.getAttendance(params).subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage.set(err?.translatedMessage || err?.error?.detail || err?.error?.message || 'حدث خطأ في تحميل سجل الحضور');
  ...
  loadSummary(): void {
    ...
    this.attendanceService.getSummary(fromDate, toDate).subscribe({
      ...
      error: (err) => {
        ...
        this.summaryError.set(err?.translatedMessage || err?.error?.detail || err?.error?.message || 'تعذر تحميل ملخص الحضور');
      }
    });
  }
