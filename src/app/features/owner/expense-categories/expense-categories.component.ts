...
import { ExpenseCategory, CreateExpenseCategoryRequest } from '../../../shared/models/gym-management.models';
...
@Component({
  ...
  template: `
    <div class="gym-page">
      <app-page-header title="فئات المصروفات" subtitle="تصنيف المصروفات التشغيلية"
        [breadcrumbs]="[{label:'لوحة التحكم', route:'/owner/dashboard'},{label:'فئات المصروفات'}]">
      ...
      <div class="state-card error-state" *ngIf="!loading() && errorMessage()" role="alert">
        ...
        <div><strong>تعذر تحميل فئات المصروفات</strong><p>{{ errorMessage() }}</p>
      ...
      <div class="data-card" *ngIf="!loading() && !errorMessage()">
        <p-table [value]="items()" [paginator]="true" [rows]="10">
          <ng-template pTemplate="header">
            <tr><th>الاسم</th><th>الوصف</th><th>الفئة الأب</th><th>الحالة</th><th style="width:120px">إجراءات</th></tr>
          </ng-template>
          ...
          <ng-template pTemplate="emptymessage">
            <tr><td colspan="5"><div class="empty-state"><i class="pi pi-tag"></i><p>لا توجد فئات</p></div></td></tr>
          </ng-template>
        </p-table>
      </div>
    </div>
    ...
    <p-dialog [(visible)]="dialog" [modal]="true" [style]="{width:'480px'}"
              [header]="isEdit ? 'تعديل الفئة':'فئة جديدة'" [dismissableMask]="true">
...
export class ExpenseCategoriesComponent implements OnInit {
  ...
  form: CreateExpenseCategoryRequest = { name: '', isActive: true };
  parentOptions = computed(() =>
    this.items().filter(c => c.id !== this.editingId).map(c => ({ label: c.name, value: c.id })));

  ngOnInit() { this.load(); }
  load() {
    this.loading.set(true);
    this.errorMessage.set(null);
    this.svc.listExpenseCategories().subscribe({
      next: d => { this.items.set(d || []); this.loading.set(false); },
      error: () => {
        this.errorMessage.set('تعذر الاتصال بخدمة فئات المصروفات. تحقق من اتصال الخادم ثم أعد المحاولة.');
  ...
  openEdit(c: ExpenseCategory) {
    ...
    this.form = { name: c.name, description: c.description, parentCategoryId: c.parentCategoryId, isActive: c.isActive };
    this.dialog = true;
  }
  save() {
    if (!this.form.name?.trim()) { this.toast.error('الاسم مطلوب'); return; }
    this.saving.set(true);
    const obs: any = this.isEdit && this.editingId
      ? this.svc.updateExpenseCategory(this.editingId, this.form)
      : this.svc.createExpenseCategory(this.form);
    obs.subscribe({
  ...
  remove(c: ExpenseCategory) {
    Swal.fire({ title:'حذف الفئة؟', text:c.name, icon:'warning', showCancelButton:true,
      confirmButtonText:'حذف', cancelButtonText:'إلغاء', confirmButtonColor:'#ef4444'})
      .then(r => { if(r.isConfirmed) this.svc.deleteExpenseCategory(c.id).subscribe({
