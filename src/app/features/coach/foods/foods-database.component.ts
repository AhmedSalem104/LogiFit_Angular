  ...
  template: `
    <div class="foods-page">
      ...
      <div class="state-card error-state" *ngIf="!loading() && errorMessage()" role="alert">
        <i class="pi pi-exclamation-triangle"></i><div><strong>تعذر تحميل قاعدة الأطعمة</strong><p>{{ errorMessage() }}</p><button type="button" class="btn btn-secondary" (click)="loadFoods()">إعادة المحاولة</button></div>
      </div>
  ...
  loadFoods(): void {
    ...
    this.coachService.getFoods().subscribe({
      ...
      error: (err) => {
        ...
        this.foods.set([]);
        this.errorMessage.set('تعذر الاتصال بخدمة الأطعمة. تحقق من اتصال الخادم ثم أعد المحاولة.');
        this.loading.set(false);
      }
    });
  }
